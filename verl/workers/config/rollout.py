...
class RolloutConfig(BaseConfig):
    ...
    enable_dynamic_max_tokens: bool = False

    # Length log: every `length_log_interval` calls, write a (min/mean/max) summary
    # of prompt/response/total token lengths to `length_log_path`. 0 disables periodic
    # logging (still logs once on first call). null path disables file output.
    length_log_interval: int = 0
    length_log_path: Optional[str] = None
