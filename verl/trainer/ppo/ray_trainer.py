...
def _compute_branch_adv_warmup_scale(step: int, warmup_start: int, warmup_steps: int) -> float:
    """Return a scale factor in [0, 1] for branch_adv warmup scheduling.

    Phases:
      - step < warmup_start                                    -> 0.0  (delay period)
      - warmup_start <= step < warmup_start + warmup_steps     -> linear ramp
      - step >= warmup_start + warmup_steps                    -> 1.0  (full strength)
      - warmup_steps == 0                                      -> 1.0 immediately after warmup_start
    """
...
def compute_advantage(
    ...
    branch_adv_kwargs: Optional[dict] = None,
    vote_risk_kwargs: Optional[dict] = None,
) -> DataProto:
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        if branch_adv_kwargs is not None:
            grpo_extra_kwargs.update(branch_adv_kwargs)
        if vote_risk_kwargs is not None:
            grpo_extra_kwargs.update(vote_risk_kwargs)
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        # Extract per-sample acc/answer/format lists from non_tensor_batch
                        # if branch_adv or vote_risk is enabled, then compute warmup-scaled
                        # alpha/beta (EDAS) or vote-risk strength (VoteRisk).
                        ...
                        if algo_cfg.get("branch_adv_enabled", False) and not algo_cfg.get(
                            ...
                            fmt_arr = batch.non_tensor_batch.get(fmt_key)
                            if acc_arr is not None and ans_arr is not None:
                                # Reuse the same warmup schedule helper — semantically identical.
