...
try:
    ...
    _HAS_MATHRULER = False

    def _grade_answer(a: str, b: str) -> bool:  # type: ignore[no-redef]
...
def _apply_branch_advantage_adjustment(
    scores: torch.Tensor,
    index: np.ndarray,
    acc_list: list,
    answer_list: list,
    ...
    """Apply branch advantage adjustment in-place on the GRPO-normalised score tensor.

    For each group (samples sharing the same prompt uid), only wrong-answer samples
    are touched:
      - Branch A (N_w <= 1): skip.
      - Branch B (K == 1, mode collapse): apply uniform extra penalty -beta * base.
      - Branch C (K > 1, healthy diversity): apply per-sample delta = alpha * base * C_i,
        where C_i = (I_i - H) / ln(N_w) is the normalised relative surprise.
    All deltas are kappa-clipped so sign(A_final) == sign(A_orig) when kappa > 1.

    Aggregate counts/sums are written to ``out_metrics`` so the caller can forward
    them to a logger.
    """
    bsz = scores.shape[0]
    ...
    for uid, indices in id2indices.items():
        wrong_indices = [
            i
            for i in indices
            if float(acc_list[i]) == 0.0 and (format_list is None or float(format_list[i]) >= 0.5)
        ]
        ...
        answers_norm = [str(answer_list[i]).strip().lower() for i in wrong_indices]

        # Step 0: canonical deduplication using math equivalence
        answer_to_canonical: dict = {}
        canonical_answers: list[str] = []
        for ans in answers_norm:
            matched = False
            for canonical in canonical_answers:
                try:
                    if _grade_answer(ans, canonical):
                        answer_to_canonical[ans] = canonical
                        matched = True
                        break
                except Exception:
                    pass
            if not matched:
                ...
                answer_to_canonical[ans] = ans

        canonical_counts = Counter(answer_to_canonical[ans] for ans in answers_norm)
        K = len(canonical_counts)

        # Step 1: macro base scale = mean(|A_orig|)
        ...
        if K == 1:
            n_groups_b += 1
            delta_desired_list = [-beta * base] * N_w
            H = 0.0
            p_list = [1.0] * N_w
            I_list = [0.0] * N_w
            C_list = [0.0] * N_w
            if log_path:
                log_lines.append(
                    f"\n[group {uid}]  N_w={N_w}  unique_answers={K}  branch=B (mode_collapse)"
                )
                log_lines.append(f"  answer_counts={dict(canonical_counts)}")
                log_lines.append(f"  H={H:.4f}  base(mean|A|)={base:.4f}")
        else:
            n_groups_c += 1
            p_list = []
            I_list = []
            for ans in answers_norm:
                k_i = canonical_counts[answer_to_canonical[ans]]
                p_i = k_i / N_w
                I_i = -math.log(p_i)
                p_list.append(p_i)
                I_list.append(I_i)

            H = sum(-p * math.log(p) for p in (k / N_w for k in canonical_counts.values()))
            log_N_w = math.log(N_w)
            C_list = [(I_i - H) / log_N_w for I_i in I_list]
            delta_desired_list = [alpha * base * C_i for C_i in C_list]

            if log_path:
                log_lines.append(
                    f"\n[group {uid}]  N_w={N_w}  unique_answers={K}  branch=C (healthy)"
                )
                log_lines.append(f"  answer_counts={dict(canonical_counts)}")
                log_lines.append(
                    f"  H={H:.4f}  ln(N_w)={math.log(N_w):.4f}  base(mean|A|)={base:.4f}"
                )

        if log_path:
            log_lines.append("  samples:")

        for j, i in enumerate(wrong_indices):
            orig_adv = orig_advs[j]
            delta_desired = delta_desired_list[j]

            max_delta = abs(orig_adv) / kappa
            delta_final = math.copysign(min(abs(delta_desired), max_delta), delta_desired)
            ...
            if log_path:
                ans_disp = answers_norm[j][:20]
                if K == 1:
                    log_lines.append(
                        f"    idx={i} answer='{ans_disp}'  orig={orig_adv:.4f}"
                        f"  delta_desired={delta_desired:.4f}  delta_final={delta_final:.4f}"
                        f"  final={new_adv:.4f}"
                    )
                else:
                    log_lines.append(
                        f"    idx={i} answer='{ans_disp}'  orig={orig_adv:.4f}"
                        f"  p={p_list[j]:.4f}  I={I_list[j]:.4f}  C={C_list[j]:+.4f}"
                        f"  delta_desired={delta_desired:+.4f}  delta_final={delta_final:+.4f}"
                        f"  final={new_adv:.4f}"
                    )

    groups_processed = n_groups_b + n_groups_c
    if log_path and groups_processed > 0:
        header = [
            "=" * 60,
            f"=== step {step} (branch_adv) ===",
            f"  groups processed: {groups_processed}  (B={n_groups_b} C={n_groups_c} skip={n_groups_skip})",
            "=" * 60,
        ]
    ...
    out_metrics["groups_b"] = n_groups_b
    out_metrics["groups_c"] = n_groups_c
    out_metrics["groups_skip"] = n_groups_skip
    out_metrics["groups_total"] = groups_processed
    out_metrics["mean_wrong_per_group"] = (sum_wrong / groups_processed) if groups_processed > 0 else 0.0
    out_metrics["mean_unique_answers"] = (sum_k / groups_processed) if groups_processed > 0 else 0.0
    out_metrics["mean_delta_abs"] = (sum_delta_abs / n_adjusted) if n_adjusted > 0 else 0.0


# ---------------------------------------------------------------------------
# VoteRisk — advantage shaping driven by P(N_j >= N_c | K draws).
# ---------------------------------------------------------------------------
#
# Background (see experiments/voterisk/ORIGINAL_EDAS_CONFIG.md for the formal
# writeup). EDAS uses error-mode diversity (relative surprise) inside the wrong
# pool to decide which wrong samples to penalise harder. VoteRisk replaces that
# diversity signal with a vote-budget risk:
#
#   R_{K,j} = P(N_j >= N_c  |  K i.i.d. draws from Trinomial(p_c, p_j, p_o))
#
# with (n_correct + n_j + n_other) = G, Dirichlet smoothing α on each component.
# Higher R_{K,j} means a wrong answer pattern that, under K extra samples, is
# more likely to tie or beat the correct answer in majority vote.
#
# We keep the EDAS shaping scaffold (kappa-clip, base = mean|A_orig|, no sign
# flip), but replace the per-mode signal with the centred risk
#   T_i = (R_i - mean_wrong_R) / (max |R_i - mean_wrong_R| + eps)
# and apply a global scale lambda_vote. Correct trajectories are not touched.
# ---------------------------------------------------------------------------


# Precomputed multinomial coefficient table. K_target is small (<= 32), so we
# build C(K, a, b) = K! / (a! b! (K-a-b)!) lazily and cache by K. This keeps the
# inner loop over (a, b) free of lgamma calls during training.
_MULTINOM_CACHE: dict[int, np.ndarray] = {}


def _get_multinomial_table(K: int) -> np.ndarray:
    """Return a (K+1, K+1) array of multinomial coefficients C(K, a, b).

    Entry (a, b) is 0 when a + b > K; otherwise K! / (a! b! (K-a-b)!).
    """
...
def _vote_risk_single_mode(
    ...
    """Exact P(N_j >= N_c | K_target i.i.d. draws from Trinomial(p_c, p_j, p_o)).

    Smoothing: p_c = (n_c + α) / (G + 3α), same Dirichlet-style α on all three
    categories. With K_target tiny (<= 32) and the multinomial table cached,
    the inner double loop is cheap enough to call once per wrong-mode per group.
    """
    p_c = (n_correct + alpha_smooth) / (G + 3.0 * alpha_smooth)
    p_j = (n_j + alpha_smooth) / (G + 3.0 * alpha_smooth)
    p_o = (G - n_correct - n_j + alpha_smooth) / (G + 3.0 * alpha_smooth)

    tab = _get_multinomial_table(K_target)
    s = 0.0
    for a in range(K_target + 1):
        # b such that a + b <= K_target. We need b >= a (mode j beats/ties correct).
        b_start = a
        b_end = K_target - a  # inclusive
        for b in range(b_start, b_end + 1):
            coeff = tab[a, b]
            if coeff <= 0.0:
                continue
            s += coeff * (p_c**a) * (p_j**b) * (p_o ** (K_target - a - b))
    return float(s)


def _apply_vote_risk_advantage_adjustment(
    scores: torch.Tensor,
    index: np.ndarray,
    acc_list: list,
    answer_list: list,
    ...
    """Apply VoteRisk advantage adjustment in-place on the GRPO-normalised score tensor.

    For each group (samples sharing the same prompt uid):

      - skip if N_w <= 1, n_c == 0, or n_c == G
      - canonical-dedup wrong answers via ``_grade_answer``
      - for each canonical mode j with n_j samples, compute R_{K,j}
      - centre: R_bar = mean(R_j);  T_j = (R_j - R_bar) / (max|R - R_bar| + eps)
      - delta_desired = lambda_vote * base * T_j
      - kappa-clip preserves sign(A_final) == sign(A_orig)
      - correct trajectories are not touched

    Logs + aggregates (mean/max risk, mean/max |delta|, mean_wrong_per_group)
    are written to ``out_metrics`` for the trainer to forward.
    """
    bsz = scores.shape[0]
    id2indices: dict = defaultdict(list)
    for i in range(bsz):
    ...
    for uid, indices in id2indices.items():
        ...
        # n_c within the full group, not within the wrong pool.
        n_c = sum(1 for i in indices if float(acc_list[i]) >= 1.0 - eps)
        if n_c == 0 or n_c == len(indices):
            n_groups_skip += 1
            continue

        # Canonical dedup of wrong answers via math equivalence (reuse EDAS path).
        answers_norm = [str(answer_list[i]).strip().lower() for i in wrong_indices]
        answer_to_canonical: dict = {}
        canonical_answers: list[str] = []
        for ans in answers_norm:
        ...
        canonical_counts = Counter(answer_to_canonical[ans] for ans in answers_norm)
        K_modes = len(canonical_counts)

        # If only one wrong mode, there is no relative ranking to redistribute
        # across — fall back to original DAPO advantage (no VoteRisk shaping).
        if K_modes <= 1:
            n_groups_skip += 1
            continue

        # Per-mode vote-risk.
        per_mode: list[tuple[str, int, float]] = []  # (canonical, n_j, risk)
        for canon, n_j in canonical_counts.items():
            risk = _vote_risk_single_mode(
                n_correct=n_c,
                n_j=n_j,
                G=G,
                K_target=K_target,
            ...
            per_mode.append((canon, n_j, risk))

        risks = [r for _, _, r in per_mode]
        mean_risk = sum(risks) / len(risks)
        max_abs_dev = max(abs(r - mean_risk) for r in risks) if risks else 0.0
        if max_abs_dev < eps:
            # All wrong modes have identical risk → no useful re-weighting.
            n_groups_skip += 1
            continue

        # EDAS-style base = mean |A_orig| over wrong samples.
        orig_advs = [float(scores[i].item()) for i in wrong_indices]
        base = sum(abs(a) for a in orig_advs) / N_w
        ...
        for r in risks:
            sum_risk += r
            sum_risk_sq += r * r
            max_risk_global = max(max_risk_global, r)

        # Build per-sample signal. Each wrong sample inherits its mode's risk.
        per_sample_signal: list[float] = []
        per_sample_delta_desired: list[float] = []
        for ans in answers_norm:
            canon = answer_to_canonical[ans]
            r_j = next(risk for c, _, risk in per_mode if c == canon)
            T = (r_j - mean_risk) / (max_abs_dev + eps)
            per_sample_signal.append(T)
            per_sample_delta_desired.append(lambda_vote * base * T)

        if log_path:
            log_lines.append(
                f"\n[group {uid}]  N_w={N_w}  unique_answers={K_modes}  n_c={n_c}  G={G}"
            )
            log_lines.append(f"  base(mean|A|)={base:.4f}  mean_risk={mean_risk:.4f}")
            log_lines.append(f"  per_mode_risks={dict((c, round(r, 4)) for c, _, r in per_mode)}")
            log_lines.append("  samples:")

        # Apply kappa-clip + write back. The caller passes kappa = 2.0 in the
        # default config (kept consistent with EDAS). EDAS shrinks the per-sample
        # delta by min(|delta_desired|, |A_orig|/kappa) and preserves sign. We
        # follow that exactly so that sign(A_final) == sign(A_orig) and the only
        # difference between EDAS and VoteRisk is the per-mode signal.
        kappa = 2.0  # matches EDAS default; user instructions keep parity.

        for j, i in enumerate(wrong_indices):
            orig_adv = orig_advs[j]
            delta_desired = per_sample_delta_desired[j]
            ...
            if log_path:
                ...
                log_lines.append(
                    f"    idx={i} answer='{ans_disp}'  orig={orig_adv:.4f}"
                    f"  signal={per_sample_signal[j]:+.4f}"
                    f"  delta_desired={delta_desired:+.4f}  delta_final={delta_final:+.4f}"
                    f"  final={new_adv:.4f}"
                )

    if log_path and n_groups_processed > 0:
        header = [
            "=" * 60,
            f"=== step {step} (vote_risk) ===",
            f"  groups processed: {n_groups_processed}  skip={n_groups_skip}",
            f"  G={G}  K_target={K_target}  alpha_smooth={alpha_smooth}  lambda_vote={lambda_vote}",
            "=" * 60,
        ]
        full_log = "\n".join(header + log_lines) + "\n"
        log_dir = os.path.dirname(os.path.abspath(log_path))
    ...
    out_metrics["mean_wrong_per_group"] = (sum_wrong / n_groups_processed) if n_groups_processed > 0 else 0.0
    out_metrics["mean_risk"] = (sum_risk / n_groups_processed) if n_groups_processed > 0 else 0.0
    out_metrics["max_risk"] = float(max_risk_global)
    out_metrics["mean_delta_abs"] = (sum_delta_abs / n_adjusted) if n_adjusted > 0 else 0.0
    out_metrics["max_delta_abs"] = float(max_delta_abs)
    out_metrics["mean_delta"] = (sum_delta / n_adjusted) if n_adjusted > 0 else 0.0
...
def compute_grpo_outcome_advantage(
    ...
    format_list: Optional[list] = None,
    branch_adv_enabled: bool = False,
    branch_adv_alpha: float = 0.2,
    branch_adv_beta: float = 0.2,
    branch_adv_kappa: float = 2.0,
    branch_adv_log_path: Optional[str] = None,
    branch_adv_step: int = 0,
    branch_adv_out_metrics: Optional[dict] = None,
    vote_risk_enabled: bool = False,
    vote_risk_G: int = 10,
    vote_risk_K_target: int = 16,
    vote_risk_alpha_smooth: float = 0.5,
    vote_risk_lambda: float = 0.4,
    ...
    Args:
        ...
        branch_adv_* : EDAS shaping (paper defaults α=0.2, β=0.2, κ=2.0).
        vote_risk_* : VoteRisk shaping. Only one of branch_adv_enabled /
                      vote_risk_enabled should be true at a time (callers in
                      recipe/dapo gate this via the launch script).
    ...
    with torch.no_grad():
        ...
        # Optional advantage shaping after GRPO standardization.
        # EDAS and VoteRisk share the same GRPO scaffold; the launcher chooses
        # which one to apply via algo_cfg.branch_adv_enabled /
        # algo_cfg.vote_risk_enabled. They are mutually exclusive — see
        # recipe/dapo/dapo_ray_trainer.py:609-666.
        has_lists = (
            acc_list is not None
            and answer_list is not None
            and len(acc_list) == bsz
            and len(answer_list) == bsz
        )

        # VoteRisk takes precedence when both flags are on (defensive). The
        # trainer should not enable both simultaneously.
        if vote_risk_enabled and has_lists:
            metrics_sink = branch_adv_out_metrics if branch_adv_out_metrics is not None else {}
            _apply_vote_risk_advantage_adjustment(
                ...
                G=vote_risk_G,
                K_target=vote_risk_K_target,
                alpha_smooth=vote_risk_alpha_smooth,
                lambda_vote=vote_risk_lambda,
                log_path=branch_adv_log_path,
                step=branch_adv_step,
                out_metrics=metrics_sink,
            )
        elif branch_adv_enabled and has_lists:
            metrics_sink = branch_adv_out_metrics if branch_adv_out_metrics is not None else {}
            _apply_branch_advantage_adjustment(
                scores=scores,
                index=index,
                acc_list=acc_list,
                answer_list=answer_list,
                format_list=format_list,
                alpha=branch_adv_alpha,
                beta=branch_adv_beta,
                kappa=branch_adv_kappa,
                log_path=branch_adv_log_path,
                step=branch_adv_step,
                out_metrics=metrics_sink,
            )
