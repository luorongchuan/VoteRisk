...
class AlgoConfig(BaseConfig):
    ...
    # ── branch_adv (EDPO) ──────────────────────────────────────
    # Branch advantage adjustment for wrong-answer diversity in GRPO.
    # See examples/branch_adv/branch_adv_implementation.md for the algorithm.
    # The reward function must return per-sample "accuracy" and "answer_pred"
    # (and optionally "format") in the result dict so they can be retrieved
    # from non_tensor_batch by ray_trainer.
    ...
    branch_adv_warmup_start: int = 0
    branch_adv_warmup_steps: int = 0
    branch_adv_acc_key: str = "accuracy"
    branch_adv_answer_key: str = "answer_pred"
    branch_adv_format_key: str = "format"

    # ── vote_risk (VoteRisk-16 advantage shaping) ──────────────────
    # VoteRisk replaces the EDAS error-diversity signal with the probability
    # that a wrong-answer mode ties or beats the correct answer under a K-sample
    # majority vote. See experiments/voterisk/ORIGINAL_EDAS_CONFIG.md and the
    # `_apply_vote_risk_advantage_adjustment` docstring in
    # verl/trainer/ppo/core_algos.py for the algorithm. Mutually exclusive
    # with branch_adv_enabled at the trainer level (only one should be on).
    vote_risk_enabled: bool = False
    vote_risk_G: int = 10
    vote_risk_K_target: int = 16
    vote_risk_alpha_smooth: float = 0.5
    vote_risk_lambda: float = 0.4
    vote_risk_warmup_start: int = 0
    vote_risk_warmup_steps: int = 0
